...
const tabsList = [
  ...
  class AppStore extends Component{
    state={isSelectedCategory:tabsList[0].tabId,userSearchInput:''}

    onSelectedButton=(tabId)=>{
        
        this.setState({isSelectedCategory:tabId})
    ...
    getFilteredItems=()=>{
        ...
        const res=appsList.filter((each)=>each.category===isSelectedCategory)
    ...
    render(){
        ...
        const{isSelectedCategory,userSearchInput}=this.state
       
        const filteredOne=this.getFilteredItems()

        const userSearchFiltered=filteredOne.filter((eachItem)=>eachItem.appName.toLowerCase().includes(userSearchInput.toLowerCase()))

        return(
            <div className='bg-container'>
                <h1 className='header'>App Store</h1>
                <div className='search-con'>
                <input type='search' onChange={this.onSearchInput} placeholder='Search' className='input-ele'/>
                <img src="https://assets.ccbp.in/frontend/react-js/app-store/app-store-search-img.png" alt="search icon" className='search-img'/>
                </div>
                ...
                <ul className='appitems-con'>
                    ...
                    <AppItem appItemList={each} key={each.appId}/>)}
